...
def get_file(file_dir):
    ...
    for file in os.listdir(file_dir + '/部落艺术'):
        expressionism.append(file_dir + '/部落艺术' + '/' + file)
        label_expressionism.append(0)
    for file in os.listdir(file_dir + '/古典主义'):
        surrealism.append(file_dir + '/古典主义' + '/' + file)
        label_surrealism.append(1)
    for file in os.listdir(file_dir + '/极简主义'):
        fauvism.append(file_dir + '/极简主义' + '/' + file)
        label_fauvism.append(2)

    # 打印出提取图片的情况，检测是否正确提取
    print("There are %d expressionism\nThere are %d surrealism\nThere are %d fauvism\n" % (
    len(expressionism), len(surrealism), len(fauvism)), end="")
    ...
    image_list = np.hstack((expressionism, surrealism, fauvism))
    label_list = np.hstack((label_expressionism, label_surrealism, label_fauvism))
    # 利用shuffle，转置、随机打乱
...
#     BATCH_SIZE = 6
#     CAPACITY = 64
#     train_dir = 'C:/Users/user/Desktop/stat3011_proj2/data'
...
#     lists = ('expressionism', 'surrealism', 'fauvism')
